...
Current released core version: **`1.10.24`**
...
- **Manual LLM Trigger (Scenario 1)**: Due to heavy memory and API overhead, Fundamental Thesis evaluation is NOT scheduled. It is strictly triggered manually by the user via the `/verify_thesis <symbol>` Discord slash command. This directly triggers the Edge Scraper to fetch the SEC EDGAR report (up to 10,000 characters) and sends it to the LLM.
