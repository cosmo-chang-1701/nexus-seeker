...
Current released core version: **`1.13.16`**
...
- **UOA 訂單流強化**：
  ...
  - **交易時段進度正規化 (`paced_ratio`)**：`_process_uoa_candidate_rows()` 額外輸出 `paced_ratio = ratio / market_time.get_trading_day_elapsed_fraction()`（同一次 `detect_uoa()`/`detect_uoa_with_physical_caps()` 呼叫內所有候選列共用同一個時段進度快照，避免逐列查詢 NYSE 行事曆）。動機：OI 是前一交易日收盤的固定值，Volume 卻隨盤中時間持續累積，同一個原始 `ratio` 在開盤 10 分鐘與收盤前 10 分鐘代表的異常程度並不相同；`paced_ratio` 是「以目前速度外推至全天收盤」的預估值，讓門檻在一天中任何時間點的意義趨於一致（開盤後 5% 交易時間內鉗制正規化分母下限，避免早盤極短時間內分母趨近零造成失真倍數）。**刻意不取代**既有 `ratio`——SWEEP/BLOCK 分類、STO 物理封頂偵測、進場鐵律條件三/四等既有已校準門檻全部維持原始 `ratio` 語意不變，避免無預警改變多個已上線判定的觸發手感；目前僅 `structural_signals._detect_whale_put_bto_block()`（SL-主力對沖）採用 `paced_ratio`（未提供時 fail-safe 退回 `ratio`）。這項正規化解決的是「原始比值的可比較性」，OI 本身仍是前一日收盤值，並未變成即時數據。
...
- **Core Account Settings (`/settings`)**: Tracks high-level financial parameters saved in the `user_settings` table:
  ...
  - `trading_strategy` (`RIGHT_SIDE` / `LEFT_SIDE` / `DYNAMIC`, migration `v068`, default `RIGHT_SIDE`) — see the **Trading Strategy Modes & 4-Regime Router** section. This is the first setting that is neither a boolean toggle nor a free-text numeric field, so `AccountSettingsView.on_select_callback()` gained a **third branch**: a fixed 3-option `TradingStrategySelectView` that writes straight to the DB and navigates back to the parent view, with no Modal (patterned on `ui/watchlist_tags.py`'s Select-subclass).
...
6. **Macro Top-Escape Anticipatory Defense (宏觀逃頂前瞻防禦)** (`market_analysis/dynamic_rollover/macro_top_escape_defense.py::evaluate_macro_top_escape_defense`): The lowest-confidence, most speculative of the six scenarios — a purely probabilistic, leading-indicator score rather than the price/margin-confirmed reactive triggers Scenarios 3/4 use. Deliberately dispatched **last** in the per-user evaluation order (3→2→5→4→6) so it can never pre-empt a more certain signal on the same symbol; it still consumes the cumulative `already_flagged_symbols` set built from Scenarios 2/3/4/5.
   - **Three gates, all required**: (1) strict opt-in via `user_settings.enable_macro_top_escape_defense` (same philosophy as Scenario 5's `target_allocation_pct` — any feature that moves the user's capital defaults off); (2) `index_microstructure.evaluate_macro_top_escape_score()` must resolve to its `CRITICAL` tier, computed from up to 5 factors — VIX term-structure backwardation (`vts_ratio`), CNN Fear & Greed extreme reading, FedWatch hawkish rate-cut probability, a negative-gamma regime flag (`get_market_regime()`), and an optional satellite-portfolio "euphoria breadth" ratio (`_compute_satellite_euphoria_ratio()`, using the `_PROFIT_UNLOCK_TOLERANCE`/`_EUPHORIA_SKEW_PERCENTILE` profit-unlock/euphoria-skew formulas — now exclusive to this scenario since Scenario 3's own take-profit gating was replaced by the Microstructure Exit Decision Matrix's TP1/TP2 tiers — with at least 3 of these factors required to align); (3) exclusion of `already_flagged_symbols`.
   - **Scope & action**: SATELLITE holdings only (same as Scenarios 3/4; CORE excess is Scenario 5's job). Trims a bounded `_MACRO_TOP_ESCAPE_TRIM_RATIO` (25%) of the position — deliberately far lower than Scenario 3's SL-tier 100% or Scenario 4's 100%, since no individual structural breakdown has actually been confirmed yet and false-positive risk is materially higher than the two reactive, price/margin-confirmed scenarios.
...
- **DTE Three-Tier State Machine (`structural_signals.evaluate_option_dte_tier(dte, position_intent)`)**: Replaces the earlier "0/1 DTE Risk-Parity Dynamic Sizing" mechanism (which widened the stop to $3.0 \times ATR_{15m}$ and halved position sizing) with an explicit three-tier classification, parameterized by whether the caller is evaluating a **new** entry/rollover (`NEW_OPPORTUNITY`) or **existing**-position risk management (`MANAGE_EXISTING`). Applies only to `OPTIONS` positions; `SPOT`/`HOLDING` `dte` defaults to a `99` sentinel and always resolves `NORMAL_EXECUTION`.
  ...
  - **`LOCKOUT_SKIP`** (`1 < dte < 7`, `NEW_OPPORTUNITY` only): The position is "end-of-life liquidity noise" — Scenario 2 (`evaluate_opportunity_cost_for_satellites`) silently skips it (no rollover computed from a decaying near-expiry contract). This tier only governs *opening a new structure*; Scenario 3's Microstructure Exit Decision Matrix manages *existing* positions (`MANAGE_EXISTING`) and is unaffected — `MAINTAIN_RISK_MONITORING` (the `MANAGE_EXISTING` counterpart for the same DTE range) intentionally imposes no restriction, so a near-expiry holding still gets protected on the way down.
...
- **Asset Class Dual-Track Exit Mechanism (現貨與期權雙軌裁決機制)**:
  - **SPOT Assets (15m Candle Close Track)**: A liquidation (`LIQUIDATE`) directive is strictly executed only if the 15-minute candle close (`price_15m_close`) falls below the calculated stop loss. Intraday lower wick breaches are treated as market maker noise and held as `HOLD` (推播安心防守卡).
    - ⚠️ **`price_15m_close` used to be populated with the live spot price**, not an actual 15m close (`_build_symbol_metrics` hardcoded `"price_15m_close": spot_val`), which quietly defeated this entire wick-filtering design — any intrabar dip counted as a body breakdown. It is now sourced from `get_confirmed_15m_bar()`, reusing the very same call `_build_symbol_metrics` already makes for the VWAP-loss/reclaim signals (zero extra network cost), with a documented fallback to spot when no confirmed bar is available yet. `price_15m_open` is threaded alongside it for the Transition Engine's bearish-body check. The two fetches sit in **separate `try` blocks** so an unrelated Session VWAP failure can no longer silently degrade the stop-loss input for every SPOT position.
  - **OPTIONS Contracts (3-5m Fast Track)**: To prevent catastrophic Delta collapse and Vega crush, options contracts bypass the 15-minute candle close wait. If spot price breaches the stop loss (instant, no wait) or an IV crash occurs ($\Delta IVR \ge 20\%$), the engine immediately triggers fast market/limit exit (`LIQUIDATE` / `STC` / `BTC`).
...
- **Microstructure Exit Decision Matrix (微觀結構出場決策矩陣)**: Replaces the earlier binary "Call Wall Euphoria 90/10" take-profit split and the whale-detection component of the old structural-breakdown gate with a 4-tier stop-loss + 3-tier take-profit ladder, evaluated by `_evaluate_microstructure_sl_ladder()` / `_evaluate_microstructure_tp_ladder()` (`anti_washout.py`). Stateless: since there is no persisted "which tier already fired" state across the 15-minute cadence, the TP ladder always reports the single **highest** currently-true tier (priority TP3 > TP2 > TP1) rather than accumulating partial exits across cycles.
  - **SL-結構失效** (100% liquidate): the Track 1 stop above (`anchor_wall - 0.5×ATR_15m`) breached — 15m close for SPOT, instant for OPTIONS.
  - **SL-狀態翻轉** (100% liquidate): the symbol's own Net GEX (`gex_profile_data.net_gex`, already fetched by `fetch_symbol_gex_metrics()` and threaded through unchanged — no new fetch) flips to `<= 0`, signaling the dealer's hedging regime has flipped from positive to negative gamma.
  - **SL-主力對沖** (100% liquidate): a real near-ATM (within 5%) single PUT BTO print with notional `>= $500k` and `ratio (Volume/OI) >= 1.5x` is detected in the symbol's UOA list (`structural_signals._detect_whale_put_bto_block()`), replacing the old proxy-only `is_whale_sto_block = (sqz_mom<0) and (skew<-0.3)` heuristic. This same function backs both Scenario 3 and Scenario 4's (`_evaluate_structural_no_edge`) whale-block signal, so both scenarios always agree on the same threat. The ratio check prefers `paced_ratio` (trading-day-elapsed-time-normalized, see §"交易時段進度正規化" above) over raw `ratio`, falling back to `ratio` when `paced_ratio` is absent.
  - **SL-動態保本** (HOLD, moves the stop): once spot has closed `>= 50%` of the distance from the anchor wall to the Call Wall, the stop is raised to `max(avg_cost, anchor_base)` (falls back to `anchor_base` alone for OPTIONS, which have no single-lot cost basis) to lock in a breakeven floor.
  - **TP1-阻力初探** (liquidate 50%): spot reaches `Call Wall × 99.5%`.
  - **TP2-空間擴展** (liquidate 30%): spot has broken above Call Wall by `>= 1.5%`. (A second sub-condition — "migrated to a new Call Wall level" — needs a cross-cycle Call Wall snapshot and is not yet implemented; only the wall-break-percentage condition ships today.)
  - **TP3-終局平倉** (liquidate 20%): any of — option Delta `>= 0.85` (computed via `market_analysis.greeks.calculate_greeks()` from the IV already fetched for the position's own quote lookup, so it costs no extra network call); a 15m VWAP loss confirmed with above-average volume (`vwap_utils.fetch_session_vwap()` + `price_volume_alert.get_confirmed_15m_bar()`, reusing that module's existing 20-bar volume-surge convention); or `0 < DTE <= 5` (the DTE<=1 forced-settlement override always fires first and is unaffected).
  - Track 2's extreme-tick-breach backstop (above) and the OPTIONS IV-crash fast exit (`ivr_drop >= 20%`) both sit **outside** this matrix and are evaluated independently — the matrix does not cover black-swan liquidity gaps or Vega/IV-crush risk.
  - `RolloverInstruction.exit_tier` (optional) records which tier fired (`SL_STRUCTURAL` / `SL_REGIME_FLIP` / `SL_WHALE_PUT` / `SL_TRAILING_BREAKEVEN` / `TP1` / `TP2` / `TP3` / `EXTREME_TICK_BREACH` / `IVR_FAST_EXIT` / `None`) for future analytics; it does not affect embed rendering, which still keys purely off `scenario`+`action`.
...
## Trading Strategy Modes & 4-Regime Router (交易策略：左側／右側／動態調整)
...
1. **結構性空頭力竭與極值乖離**: `Spot <= VWAP − 1.5×ATR₁₅ₘ`, 15m RSI ≤ 30, a bullish-reversal candle pattern, and either volume exhaustion (≤0.7× avg) or panic absorption (≥2.0× avg without closing at the low).
2. **做市商 Put Wall 密著截擊**: spot within `[−1.0%, +1.5%]` of the Put Wall, plus a defense-thickness check.
...
- Condition 4's spec calls for a "within the past 4 hours" window. The UOA list is a **whole-day option-chain snapshot** (`volume` is cumulative day volume, `oi` is prior-close open interest), not a time-and-sales tape, so no per-print timestamp exists or can exist here. `paced_ratio` in the same module exists precisely to compensate for this data model. Documented in the condition's docstring so it doesn't read as an oversight.

### 3. `動態調整` (DYNAMIC) — `market_analysis/dynamic_rollover/regime_classifier.py`
`classify_dynamic_regime()` classifies the tape into one of four regimes and routes accordingly. Fail-safe: any missing data resolves to Regime II (stand aside — neither enter nor force-liquidate).
...
Regime II is deliberately implemented as the `else` fallback rather than checking the spec's `Put Wall < Spot < Gamma Flip` band: classification must be total, and a band check would leave states outside it with nowhere to land. The fallback direction is the safe one (no action).
...
- **`trim_to_confirmed_15m_bars(df)` is the single shared definition of that truncation**, extracted so `get_confirmed_15m_bar` and the `dynamic_rollover` left-side gate / regime classifier cannot drift apart. It returns the frame truncated at the last closed bar (or `None` when that leaves too few bars for the 20-bar volume baseline, or when the index isn't datetime-typed — fail closed rather than assuming "closed"). Using a still-forming bar is especially dangerous for *shrinking*-volume conditions such as the left-side gate's 縮量窒息 (`volume <= 0.7× avg`): a bar 2 minutes into its 15-minute window has only accumulated a fraction of its volume, so that sub-condition would read as true for most of every bar.
