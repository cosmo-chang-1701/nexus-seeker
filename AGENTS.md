...
- 互動設定與通知偏好中心（4 模組 19 頻道；頻道註冊表 `database/notification_channels.py`、集中推播入口 `services/notification_dispatcher.py`）→ [`03_notification_center.md`](docs/platform/03_notification_center.md)
...
- `nexus_core/database/notification_channels.py` — 通知頻道**單一註冊表**：key／模組／標籤／預設值／各預設情境開關只在 `CHANNELS` 定義，`ALL_NOTIFICATION_KEYS`／`PRESET_PROFILES`／`TRADING_MODULES` 由此衍生；`NotificationKey` Literal 供 mypy 攔下拼錯的頻道名
