...
**資料來源**：UOA 歷史由 15 分鐘心跳在計算完 UOA 後一併寫入 `uoa_history`（與既有的 `kv_cache` 快取共用同一份資料，零額外期權鏈抓取成本）。`kv_cache` 的 `uoa_{SYMBOL}` 是 `ON CONFLICT DO UPDATE` 的 upsert，每輪覆蓋前一輪，結構上無法回看，因此必須另立資料表。保留期 10 個交易日（覆蓋 5 個交易日窗並留一倍緩衝，以交易日計算，不必猜測連假長度），由 03:00 ET 離峰排程清理。
