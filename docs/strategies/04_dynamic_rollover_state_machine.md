...
判讀指標依重要性排序：**索提諾比率 (Sortino) 為主，最大回撤 (MDD) 與 VaR / CVaR 為輔**（定義見 [`../risk_portfolio/07_downside_risk_sortino_var_cvar.md`](../risk_portfolio/07_downside_risk_sortino_var_cvar.md)；MAR = 無風險利率 4.5%）。夏普比率對上下行波動一視同仁，會把「砍掉獲利部位」誤判為風險改善，**只保留為描述欄位，不作任何判讀依據**。

以下為 **2026-09-23 以現行程式碼重跑**的結果（`calibration/backtest_engine_2025.py` 改用 `market_analysis/downside_risk.py` 之後；總報酬與 MDD 不受指標定義影響，與早期版本 +16.64% / +14.61% 的差異來自此後引擎本身的演進）：

| 判讀指標 | 穩健防禦型 (Defensive) | 動能進攻型 (Aggressive) | 靜態持有基準 (Buy & Hold) |
| :--- | :---: | :---: | :---: |
| **索提諾比率 (Sortino, MAR=4.5%)** | **1.45** | **1.13** | **1.73** |
| **最大回撤 (MDD)** | **12.50%** | **11.77%** | **16.80%** |
| **1 日 CVaR95** | **1.59%** | **1.56%** | **2.94%** |
| **1 日 VaR95** | **0.90%** | **0.91%** | **1.91%** |
| **超額報酬 vs 減碼 B&H（下行差對齊）** | **−2.08 pp**（$w$ = 55.5%） | **−4.44 pp**（$w$ = 54.5%） | — |
| 年化下行差 (MAR=4.5%) | 7.58% | 7.44% | 13.65% |
| 全年度總報酬率 | +15.33% | +12.72% | +27.77% |
| 複合年化報酬率 (CAGR) | +15.52% | +12.88% | +28.15% |

| 描述性指標（不作判讀） | 穩健防禦型 | 動能進攻型 | Buy & Hold |
| :--- | :---: | :---: | :---: |
| 夏普比率 (Rf=4.5%) | 1.01 | 0.78 | 1.18 |
| 卡瑪比率 (CAGR/MDD) | 1.24 | 1.09 | 1.68 |
| 年化波動率 | 10.89% | 10.69% | 20.10% |
| 已實現勝率 | 90.4% | 80.0% | N/A |
| 獲利因子 | 3.28 | 1.85 | N/A |
| 全年總調度筆數 | 99 筆 | 127 筆 | 0 筆 |

> ⚠️ **以 Sortino 判讀，現行程式碼下 Defensive 優於 Aggressive**（1.45 vs 1.13），兩者都低於 B&H。`RiskAppetite.AGGRESSIVE` 當初是以「報酬／MDD／Sharpe 三項皆優於 Defensive」採用（下方 §2.10.2 的早期數字），此前提在現行程式碼與新判準下**不再成立**；是否調整預設或 AGGRESSIVE 參數組屬人工決策，尚未變更。唯一在 Sortino 上超越 B&H 的組合是 Aggressive + `PYRAMID_ADD`（1.87，超額報酬 +1.01 pp），見 [`05_calibration_harness_and_forward_collection.md`](../architecture/05_calibration_harness_and_forward_collection.md) §5.12。

#### 2.10.2 動能進攻型之量化機制突破

> 本節與 §2.10.3 的損益數字出自引擎早期版本的首次回測，保留作為機制說明；現行數字以 §2.10.1 為準。
...
上表的高勝率與低回撤**不代表引擎創造了 alpha**。兩個模式的年化下行差都只有 B&H 的約 55%，等於平均只承擔約一半的下行曝險。拿同等下行差的「$w$ × B&H + $(1-w)$ × 現金（$R_f = 4.5\%$）」當對照組——此混合組合的超額報酬恰為 $w \times (R_{\text{B\&H}} - R_f)$，其下行差（MAR = $R_f$）精確等於 $w$ × B&H 下行差，因此與策略承擔**完全相同的 Sortino 分母**：
...
進攻型 12.72%、Sortino 1.13，兩項都輸給這個對照組——整套情境引擎被「單純減碼持有」支配。勝率是反覆砍獲利部位造成的低賺賠比副產品，不是優勢。後續的曝險遞增三件套（TP1 趨勢豁免、§2.11 `PYRAMID_ADD`、晴空萬里天花板）與 Regime III-B 都是為了讓部位曝險在趨勢中能夠**遞增**，而非持續遞減；任何「再加一道風控閘門」方向的提案都會讓問題惡化——Sortino 不懲罰上行波動，截斷上行的閘門只會壓低分子。
...
#### 2.10.5 多資產 2022–2025 回測（VOO 核心 + 8 檔個股衛星 + GLD）

上述 §2.10.1–§2.10.4 是 2025 單年、SPY／NVDA／GLD 三標的的結果，保留作為歷史對照。為了在包含空頭年（2022）的期間、以更接近使用者實際持股的組合檢驗引擎，回測引擎已一般化為「核心 + N 檔衛星」（資料與方法見 [`05_calibration_harness_and_forward_collection.md`](../architecture/05_calibration_harness_and_forward_collection.md) §5.15）：

- 配置（策略與 B&H 共用）：VOO 核心 40%；NVDA、META、GOOGL、TSLA、MU、PLTR、FCX、MRNA 各 6%；GLD 7%；現金 5%。大盤訊號代理（負 Gamma、危機判定、逃頂、保護性 Put）維持 SPY。
- 期間 2022-01-03 → 2025-12-30。判讀以 **Sortino（MAR = 4.5%）為主**。

| 組合 | Sortino | 超額 vs 減碼 B&H | MDD | 1 日 CVaR95 | 總報酬 | 換股次數 |
| :--- | ---: | ---: | ---: | ---: | ---: | ---: |
| **B&H（單純持有）** | **1.17** | — | 33.6% | 3.52% | +139.1% | 0 |
| DEFENSIVE | 0.41 | −34.5 pp | 15.7% | 1.37% | +32.7% | 149 |
| AGGRESSIVE | 0.28 | −65.6 pp | 22.4% | 2.37% | +34.6% | 222 |
| HYBRID（aggressive + defensive 換股參數） | −0.13 | −89.9 pp | 23.8% | 2.45% | +12.4% | 144 |
| DEFENSIVE + `PYRAMID_ADD` | 0.49 | −33.3 pp | 15.9% | 1.41% | +36.1% | 145 |
| AGGRESSIVE + `PYRAMID_ADD` | 0.26 | −66.0 pp | 22.1% | 2.32% | +33.4% | 225 |
| HYBRID + `PYRAMID_ADD` | 0.03 | −78.4 pp | 23.7% | 2.33% | +20.6% | 145 |

逐年 Sortino（總報酬）：

| 組合 | 2022 | 2023 | 2024 | 2025 |
| :--- | :---: | :---: | :---: | :---: |
| **B&H** | −1.63（−29.6%） | 3.53（+41.6%） | 3.77（+55.2%） | 2.43（+54.3%） |
| DEFENSIVE | −2.18（−14.3%） | 2.28（+17.3%） | 2.53（+19.6%） | 0.82（+10.2%） |
| AGGRESSIVE | −2.16（−21.8%） | 2.63（+32.7%） | 1.65（+21.8%） | 0.16（+6.5%） |
| DEFENSIVE + `PYRAMID_ADD` | −2.23（−14.8%） | 2.09（+17.5%） | 2.02（+17.1%） | 1.54（+16.0%） |

**判讀**

1. **所有組合都被單純持有支配**：Sortino 0.41 以下（最高 0.49），B&H 為 1.17；超額報酬 vs 下行差對齊的減碼 B&H 全部為負（−33 到 −90 pp）。與 2025 單年的結論一致，且在更長期間、更多標的下更明顯。
2. **2022 空頭年也沒有贏**：引擎把 2022 的虧損從 −29.6% 壓到 −14% 至 −22%，但每承擔一單位下行風險換到的報酬反而更差（Sortino −2.2 vs −1.6）；隨後三年只拿到 B&H 報酬的三到五成。
3. **結果對參數與路徑高度敏感**：AGGRESSIVE 與 HYBRID 只差換股門檻與冷卻，Sortino 卻從 0.28 變成 −0.13；同一組合在換股頻率修正前後（§2.10.6）也大幅變動。沒有任何一組參數穩健地優於其他組；2025 單年「AGGRESSIVE + `PYRAMID_ADD` 唯一贏過 B&H」的結果在多資產、四年期間**不成立**（0.26）。
4. **逃頂三級階梯**（§5.12 代理）使 MDD 從 15.7%／22.4% 降到 11.2%／19.0%，但 Sortino 同步下降（DEFENSIVE 0.41 → −0.08，AGGRESSIVE 0.28 → 0.25）；WATCH 四年成立約 350 天，代理仍然過鬆。

#### 2.10.6 換股頻率：多衛星一般化時的語意修正

原引擎只有 NVDA ↔ GLD 一對，冷卻以「每檔來源衛星」計算。直接套用到 9 檔衛星時，四年換股 374（DEFENSIVE）／512（AGGRESSIVE）次，每年約 100 次，是 2025 三標的（每年 12–19 次）的 8–10 倍。拆解後的主因：

- 9 檔各自獨立的冷卻 → 同一天最多有 5 檔同時換出（2022-01-03 五檔同時全數轉入 MRNA，約 30% 的投組一天內集中到單一標的）；
- 剛轉入的標的數日內又被轉出（5 日內反轉 54／108 次）。

這偏離了原設計「衰退衛星 → 突破標的、單向、低頻」的語意。多資產配置改用**投組層級輪動**（`BacktestUniverse.rotation_policy = "portfolio"`）：每天最多一筆（所有「衰退 → 突破」組合中 EV 價差最大者），冷卻以投組計算。兩檔衛星時原規則本身就只有一對、兩方向同日互斥，因此三標的配置維持原規則，回歸不變式不受影響。修正後換股降為 149／222 次（每年 37／56 次），仍高於三標的版本，這是標的數增加的自然結果。
