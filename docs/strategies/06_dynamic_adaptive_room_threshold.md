...
**適用市場環境**：全域。三套進場鐵律（右側動能、左側均值回歸、做空破位）、六態 Regime 路由分類器、Gamma Squeeze SPEAR 磁吸目標價，以及分析中心的 GEX 空間欄位，全部共用同一條演算法。共用的是**演算法**而非常數值——各站點仍各自獨立呼叫、各自持有輸入，路由層與進場確認層的職責邊界不因此被破壞。
