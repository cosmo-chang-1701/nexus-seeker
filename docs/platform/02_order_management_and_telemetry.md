...
### 4.1 待決事項：委託單遙測的 Skew 輸入
`services/order_telemetry_service.py` 的一鍵重算路徑目前**不是**讀取真實 Skew 百分位，而是把原始 Skew 值換算成代理值：Skew $> 5$ 個百分點視為 98、$< -2$ 視為 2、其餘為 50。另一條「一鍵套用」後備路徑則把 Skew 寫死為 98。量綱統一時刻意保留了這個行為，因為改用真實分位會改變尾端防禦的觸發頻率（日級母體下 $\ge 95$ 的觸發率約 9%，舊代理的觸發條件是絕對的 5 個百分點）。

是否改為讀取 `calculate_skew()` 回傳的 `skew_percentile`，需要先比較兩者在自選標的上的觸發頻率，再由人工決定；寫死 98 的後備路徑則應在改用真實值時一併移除。
