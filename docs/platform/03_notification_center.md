...
#### 2.2.5 UI（`NotificationSettingsView`，5 row 以內）
  ...
  - Row 4：
    - `📉 投組下行風險`：另發一則僅自己可見（ephemeral）的快照，列出 Sortino（一季／一年，主指標）、最大回撤與距高點回撤、1 日 VaR95／CVaR95；已實現淨值滿 60 個交易日後並列顯示（資料來源與 `risk_portfolio_downside` 推播、盤後戰報相同：`services/downside_risk_service.get_downside_snapshots()`）。快取未命中時需抓一年日線，因此先 `defer`；快取未命中且 `is_memory_safe()` 為否時不抓取，只回報原因。設定面板本身不變。
    - `⚙️ 進階：情報與戰報`／`⬆️ 收合進階` 切換按鈕。展開時直接切到第一個進階模組；收合時若正停在進階模組則回到左尾防護，停在核心模組則保持不變。
