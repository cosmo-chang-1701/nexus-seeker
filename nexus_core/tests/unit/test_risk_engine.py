...
def test_get_macro_modifiers_all_cases():
    # VIX tiers
    assert get_macro_modifiers(MacroContext(vix=35.0, oil_price=70.0, vix_change=0.0))[0] == 2.0
    assert get_macro_modifiers(MacroContext(vix=30.0, oil_price=70.0, vix_change=0.0))[0] == 1.5
    ...
    assert w_regime == 1.0
    _, _, w_regime = get_macro_modifiers(MacroContext(vix=20.0, oil_price=70.0, vix_change=0.0, vts_ratio=1.1, vix_trend_up=False))
    assert w_regime == 0.6
    _, _, w_regime = get_macro_modifiers(MacroContext(vix=20.0, oil_price=70.0, vix_change=0.0, vts_ratio=0.9, vix_trend_up=True))
    assert w_regime == 0.6
...
def test_optimize_position_risk_all_branches():
    ...
    qty_extreme, _ = optimize_position_risk(0.0, 1.0, 100000.0, 500.0, 0.2, "BTO_CALL", macro_extreme, vix_spot=36.0)
