...
# ─────────────────────────────────────────────────────────────────────────────
# create_entry_rules_embed (進場鐵律檢核頁籤：六重鐵律)
# ─────────────────────────────────────────────────────────────────────────────


def test_create_entry_rules_embed_renders_six_rule_checklist() -> None:
    """六重鐵律應渲染為獨立欄位，且 Pass/Fail 符號正確對應。"""
    ...
    embed = create_entry_rules_embed("NVDA", False, six_rule_reasons)
    ...
    assert "六重鐵律" in str(embed.footer.text)


def test_create_entry_rules_embed_handles_missing_data() -> None:
    """六重鐵律資料缺席時（例如尚未點擊過或計算失敗），不應拋出例外，
    應顯示「尚無足夠數據進行判定」佔位文字。"""
    from cogs.embed_builders.portfolio_embeds import create_entry_rules_embed

    embed = create_entry_rules_embed("NVDA", None, None)
