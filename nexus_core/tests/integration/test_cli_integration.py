...
def test_cli_portfolio_integration(db_conn, mock_market_data):
    ...
    with patch('market_analysis.portfolio.get_option_chain_mid_iv') as mock_mid:
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ['--db', config.DB_NAME, '--user-id', str(user_id), 'portfolio'])
...
def test_cli_scan_ddp_integration(db_conn):
    ...
    with patch('services.trading_service.TradingService.run_ddp_scan', new_callable=AsyncMock) as mock_scan:
