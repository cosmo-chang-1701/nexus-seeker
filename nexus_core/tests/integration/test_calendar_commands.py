...
async def test_command_calendar(mock_interaction, db_conn):
    ...
    with patch("services.calendar_service.calendar_service.get_portfolio_events", new_callable=AsyncMock) as mock_events:
        mock_events.return_value = [
            {'type': 'ECONOMIC', 'event': 'FOMC', 'impact': 'high', 'country': 'US', 'tte_hours': 24.0, 'time': '2026-05-15'},
            {'type': 'EARNINGS', 'symbol': 'AAPL', 'date': '2026-05-16', 'tte_hours': 48.0}
        ]
...
async def test_command_iv_rank(mock_interaction, db_conn):
    ...
    with patch.object(cog.vol_inspector, "run_scan", new_callable=AsyncMock) as mock_scan:
...
async def test_command_event_impact(mock_interaction, db_conn):
    ...
    add_portfolio_record(user_id, symbol, "call", 150, "2026-06-19", 5.0, 1, 150.0, weighted_delta=10.0, gamma=0.5)

    # 2. Mock market data for price
    with patch("services.market_data_service.get_quote", new_callable=AsyncMock) as mock_quote, \
         patch("market_analysis.greeks.calculate_vanna") as mock_vanna:

        mock_quote.return_value = {'c': 155.0}
        mock_vanna.return_value = 2.5 # Simulated Vanna

        await cog.event_impact.callback(cog, mock_interaction, symbol=symbol, vol_move=20.0)
