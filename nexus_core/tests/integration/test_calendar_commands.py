...
@pytest.fixture
def calendar_mock_interaction() -> Any:
...
async def test_calendar_command_high_rate(  # type: ignore
    ...
    await cog.calendar.callback(cog, calendar_mock_interaction)  # type: ignore

    calendar_mock_interaction.response.defer.assert_awaited_once_with(ephemeral=True)
    calendar_mock_interaction.followup.send.assert_awaited_once()

    call_args = calendar_mock_interaction.followup.send.call_args[1]
