...
async def test_command_runway_check(mock_interaction, db_conn):
    ...
    await cog.update_settings.callback(cog, mock_interaction, monthly_expense=5000.0, cash_reserve=20000.0)
    ...
    sent = mock_interaction.response.send_message.called or mock_interaction.followup.send.called
    assert sent
...
async def test_command_skew_scan(mock_interaction, db_conn, mock_market_data):
    ...
    with patch("market_analysis.sentiment_engine.SentimentEngine.calculate_skew", new_callable=AsyncMock) as mock_skew, \
         patch("market_analysis.sentiment_engine.SentimentEngine.calculate_pcr", new_callable=AsyncMock) as mock_pcr, \
         patch("market_analysis.sentiment_engine.SentimentEngine.detect_uoa", new_callable=AsyncMock) as mock_uoa, \
         patch("market_analysis.sentiment_engine.SentimentEngine.calculate_max_pain", new_callable=AsyncMock) as mock_mp:
...
async def test_command_ddp_scan(mock_interaction, db_conn, mock_market_data):
    ...
    with patch("market_analysis.ddp_inspector.DDPInspector.run_scan", new_callable=AsyncMock) as mock_scan:
...
async def test_command_poly_list(mock_interaction, db_conn):
    ...
    bot.polymarket_service.get_active_markets.return_value = [{"title": "Test Market", "price": 0.5}]
...
async def test_command_settle_hedge(mock_interaction, db_conn):
    ...
    await cog.settle_hedge.callback(cog, mock_interaction, alert_id=alert_id, actual_qty=12)
    
    mock_interaction.followup.send.assert_called_once()
    assert "embed" in mock_interaction.followup.send.call_args[1]
    
    cursor.execute("SELECT status, hedge_contracts FROM hedge_alerts WHERE id = ?", (alert_id,))
    row = cursor.fetchone()
