...
class CalendarCog(commands.Cog):
    ...
    @app_commands.command(name="calendar", description="顯示影響目前投資組合的即時重大事件")
    ...
    @app_commands.command(name="iv_rank", description="掃描觀察清單中具備高 IV Rank 或財報前夕的標的")
    ...
    @app_commands.command(name="event_impact", description="針對特定即時事件進行 Greeks (Delta, Vega) 的 What-if 模擬")
    @app_commands.describe(symbol="欲模擬的標的代號", vol_move="預期波動率變動 % (例如 20 代表 20%)")
    async def event_impact(self, interaction: discord.Interaction, symbol: str, vol_move: float = 20.0):
        await interaction.response.defer(ephemeral=True)
        symbol = symbol.upper()
        user_id = interaction.user.id
