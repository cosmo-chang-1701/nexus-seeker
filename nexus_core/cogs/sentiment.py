...
class SentimentCog(commands.Cog):
    ...
    @app_commands.command(name="skew_scan", description="執行期權偏斜 (Skew) 與市場情緒掃描")
    ...
    async def skew_scan(self, interaction: discord.Interaction, symbol: str):
        ...
        try:
            ...
            if "error" in skew_data and skew_data["error"] == "No expiries":
                 return await interaction.followup.send(f"❌ 無法取得 `{symbol}` 的期權數據，請檢查標的是否支援期權交易。")

            embed = create_sentiment_scan_embed(symbol, skew_data, pcr_data, uoa_data, max_pain_data)
    ...
    @app_commands.command(name="max_pain", description="計算特定標的之最大痛點 (Max Pain)")
    @app_commands.describe(symbol="股票代碼", expiry="到期日 (YYYY-MM-DD，選填，預設最近到期)")
    async def max_pain(self, interaction: discord.Interaction, symbol: str, expiry: Optional[str] = None):
        symbol = symbol.upper()
        await interaction.response.defer(ephemeral=False)
        ...
        try:
            ...
            embed.add_field(name="最大痛點 Strike", value=f"**${data['max_pain']}**", inline=True)
            embed.add_field(name="目前價格", value=f"`${data['current_price']}`", inline=True)
            
            dist = data['distance_pct']
            dist_str = f"現價高於痛點 `{dist}%`" if dist > 0 else f"現價低於痛點 `{abs(dist)}%`"
            ...
            if dte <= 3:
                embed.add_field(name="🚀 執行建議", value="⚠️ **DTE < 3 且接近最大痛點**\n建議提升 **獲利鎖定 (Profit Lock)** 優先級，規避結算震盪。", inline=False)
            ...
            await interaction.followup.send(f"❌ 執行計算時發生錯誤。")
