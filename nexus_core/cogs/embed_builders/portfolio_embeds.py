...
def create_entry_rules_embed(
    ...
    detail_lines = [
        ...
        " candidate 自身 DTE 雜訊安全閥，僅於前四項皆通過後才會啟動判定",
        " ----------------------------------",
        " ├─ 條件一・結構性右側放量突破確認",
        "     • 15m 實體K棒收盤價 > Gamma Flip 估算門檻",
        "     • 15m 成交量 ≥ 前20根均量 × 1.5倍 (放量確認)",
        "     • K棒須為實體陽線 (close > open)，排除陰線放量假突破",
        "     • 個股淨 GEX 須為 LONG_GAMMA (net_gex > 0)",
        "     • 四項同時滿足才通過，避免負Gamma泥淖下的陰線放量",
        "       被誤判為右側突破",
        " ├─ 條件二・做市商正 Gamma 底牆完好",
        "     • 掃描 GEX Profile 中最大正 GEX 履約價作為支撐牆",
        "     • 現價須 > 支撐牆 (且支撐牆須存在，即 GEX > 0)",
        "     • 未偵測到有效正 Gamma 支撐牆時直接判定未通過",
        " ├─ 條件三・UOA 無實質物理封頂",
        "     • 無單筆 STO Call ratio(成交量/OI) > 1.0x 的物理封頂",
        "     • Call Wall 距現價空間 (call_wall-現價)/現價 ≥ 5%",
        "       (帶正負號；現價已觸及或跌破 Call Wall 同樣視為",
        "       空間不足，而非「已站上、無封頂」)",
        "     • 兩項條件須同時成立",
        " └─ 條件四・主力跨週期買盤認證與雜訊過濾",
        "     • 掃描 UOA 清單 (依權利金金額/名目價值降序)",
        "     • 尋找 CALL BTO 買盤，且 DTE ≥ 7、ratio(成交量/OI) ≥ 0.8x",
        "     • 找到第一筆同時符合兩項門檻者即判定通過",
        "```",
    ]
    _add_ansi_field_safely(embed, "📖 條件一～四判定說明 (指標定義)", detail_lines)
