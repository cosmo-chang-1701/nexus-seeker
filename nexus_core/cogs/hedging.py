...
class HedgingCog(commands.Cog):
    ...
    @app_commands.describe(alert_id="對沖警報 ID (見戰位報告底部)", actual_qty="實際執行的合約/股數 (選填)")
    async def settle_hedge(self, interaction: discord.Interaction, alert_id: int, actual_qty: Optional[int] = None):
        ...
        try:
            ...
            # 1. 驗證警報是否存在且屬於該使用者
            cursor.execute("SELECT * FROM hedge_alerts WHERE id = ? AND user_id = ?", (alert_id, user_id))
            alert = cursor.fetchone()
            
            if not alert:
                return await interaction.followup.send("❌ 找不到該警報 ID，或您無權操作。", ephemeral=True)
            
            if alert[10] != 'PENDING': # status is at index 10
                return await interaction.followup.send(f"⚠️ 該警報已處理過 (狀態: {alert[10]})。", ephemeral=True)

            # 2. 更新狀態為 EXECUTED
            final_qty = actual_qty if actual_qty is not None else alert[8] # hedge_contracts is at index 8
    ...
    async def hedge_list(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not rows:
                return await interaction.followup.send("📭 目前無對沖警報紀錄。", ephemeral=True)
            ...
            content = []
            for r in rows:
                status_emoji = "⏳" if r[3] == 'PENDING' else "✅" if r[3] == 'EXECUTED' else "❌"
                content.append(f"`#{r[0]}` | {status_emoji} | VIX: `{r[1]:.2f}` | 建議: `{r[2]}`股 | {r[4][:16]}")
