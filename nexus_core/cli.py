...
def portfolio_pnl(ctx):
    ...
    async def _run():
        ...
        for t in data['trades']:
            ...
            table.add_row(
                str(t['id']), t['symbol'], str(t['quantity']),
                f"${t['entry_price']:.2f}", f"${t['current_price']:.2f}",
...
def market_quote(ctx, symbol):
    ...
    async def _run():
        ...
        panel = Panel(
            ...
            title=f"📊 {symbol_upper} 即時報價", expand=False
        )
        console.print(panel)
    run_async(_run())
...
def market_ddp(ctx):
    ...
    async def _run():
        ...
        if not results:
            ...
            return
        for res in results:
            rprint(Panel(f"標的: {res['symbol']}\nP/E: {res.get('pe', 'N/A')}\nEPS Growth: {res.get('eps_growth', 'N/A')}%", title="🎯 DDP 訊號"))
    run_async(_run())
...
def market_skew(ctx, symbol):
    ...
    async def _run():
        ...
        rprint(Panel(f"Skew 分位點: {res['skew']}%\n狀態: {res['state']}", title=f"📐 {symbol.upper()} Skew Analysis"))
    run_async(_run())
