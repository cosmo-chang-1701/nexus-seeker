...
def migrate_data(conn: sqlite3.Connection):
    ...
    try:
        ...
        if table_exists("portfolio"):
            cursor.execute(
                "SELECT user_id, symbol, opt_type, strike, expiry, entry_price, quantity, weighted_delta, theta, gamma, trade_category FROM portfolio"
            )
            for (
                uid,
                sym,
        ...
        if table_exists("holdings"):
            cursor.execute(
                "SELECT user_id, symbol, quantity, avg_cost, weighted_delta FROM holdings"
            )
            for uid, sym, qty, cost, w_delta in cursor.fetchall():
                metadata = json.dumps(
                    {"quantity": qty, "avg_cost": cost, "weighted_delta": w_delta}
                )
                cursor.execute(
