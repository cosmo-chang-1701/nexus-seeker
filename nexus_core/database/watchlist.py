...
def get_user_watchlist(user_id):
    ...
    conn = sqlite3.connect(config.DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT symbol, metadata FROM assets WHERE user_id = ? AND context_type = 'WATCH'", (user_id,))
    rows = []
...
def get_user_watchlist_by_symbol(user_id, symbol):
    ...
    conn = sqlite3.connect(config.DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT symbol, metadata FROM assets WHERE user_id = ? AND symbol = ? AND context_type = 'WATCH'", (user_id, symbol.upper()))
    rows = []
    for sym, meta_json in cursor.fetchall():
        meta = json.loads(meta_json) if meta_json else {}
        use_llm = 1 if meta.get('use_llm', True) else 0
...
def get_all_watchlist():
    ...
    conn = sqlite3.connect(config.DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, symbol, metadata FROM assets WHERE context_type = 'WATCH'")
    rows = []
...
def delete_watchlist_symbol(user_id, symbol):
    ...
    conn = sqlite3.connect(config.DB_NAME)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM assets WHERE user_id = ? AND symbol = ? AND context_type = 'WATCH'", (user_id, symbol.upper()))
