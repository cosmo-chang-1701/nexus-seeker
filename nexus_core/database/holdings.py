...
def get_user_holdings(user_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT id, symbol, metadata, created_at FROM assets WHERE user_id = ? AND context_type = 'HOLDING'", (user_id,))
    rows = []
...
def delete_holding(user_id: int, symbol: str) -> bool:
    ...
    conn = sqlite3.connect(config.DB_NAME)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM assets WHERE user_id = ? AND symbol = ? AND context_type = 'HOLDING'", (user_id, symbol.upper()))
...
def get_all_holdings():
    ...
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT id, user_id, symbol, metadata FROM assets WHERE context_type = 'HOLDING'")
    rows = []
    for row in cursor.fetchall():
        d = dict(row)
        meta = json.loads(d['metadata']) if d['metadata'] else {}
