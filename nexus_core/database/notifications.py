...
def add_pending_notification(user_id: int, content: Optional[str] = None, embed_dict: Optional[dict] = None):
    """將待發送通知存入資料庫"""
    try:
...
def get_pending_notifications(limit: int = 50) -> List[Tuple[int, int, Optional[str], Optional[dict]]]:
    """獲取待發送通知清單"""
    results = []
    try:
        with sqlite3.connect(config.DB_NAME) as conn:
