...
class AttributionEngine:
    ...
    def calculate_protection_score(
        ...
        """
        if cost_of_hedge <= 0:
            return 100.0 if loss_avoided > 0 else 0.0
    ...
    async def finalize_vtr_attribution(user_id: int, window_hours: int = 24):
        ...
        try:
            ...
            cursor.execute(
                "SELECT id, event_context, pre_hedge_greeks FROM vtr_hedge_logs WHERE user_id = ? AND status = 'OPEN'",
                (user_id,),
            )
            ...
            for log_id, context_json, greeks_json in logs:
                ...
                score = AttributionEngine.calculate_protection_score(
                ...
                """,
                    (
                        theoretical_loss_avoided - cost,
                        cost,
                        theoretical_loss_avoided,
                        score,
                        log_id,
                    ),
                )
    ...
    def format_attribution_report(user_id: int) -> List[str]:
        ...
        try:
            ...
            for ts, tag, score, loss, context_json in rows:
                ...
                lines.append(
                    f"  └ 分數: `{score:.1f}` | 避免損失: `${loss:,.0f}` | 時間: `{ts[5:16]}`"
                )
