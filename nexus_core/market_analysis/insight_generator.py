...
def compute_realtime_insights(data: Dict[str, Any]) -> str:
    ...
    if is_put_wall_crisis:
        max_pain_diff = round(dist_pct, 1)
        return f"• ⚠️ **{sym}**: 雖然股價距離 Max Pain 有 {max_pain_diff}% 的偏差，但因現價已穿透/逼近做市商 PutWall 防線，進入負 Gamma 禁區。做市商 Delta 剛性拋壓（Delta Negative Feedback Loop）已全面主導盤面。此處散戶搶購 Call 的偏斜亢奮將加劇做市商的對沖拋售壓力，嚴禁任何左側接刀行為，靜待底牆企穩。"
    ...
    if dist_pct < -10.0:
        dev_fact = (
            f"，股價距離 Max Pain 有 {dist_pct:.1f}% 的偏差，存在顯著的向上磁吸效應"
        )
    else:
