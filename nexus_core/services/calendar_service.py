...
class CalendarService:
    ...
    def __init__(self):
        ...
        self._high_impact_keywords = ["CPI", "FOMC", "Fed Interest Rate", "Nonfarm Payrolls", "Employment Situation"]
    ...
    async def get_high_impact_events(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raw_events = await market_data_service.get_economic_calendar(start_date, end_date)
            ...
            for event in raw_events:
                ...
                is_high = impact.lower() == 'high' or any(kw in name for kw in self._high_impact_keywords)
                
                if is_high:
                    event_time_str = event.get('time', '')
                    if event_time_str:
                        event_dt = datetime.fromisoformat(event_time_str.replace('Z', '+00:00'))
                        tte_hours = (event_dt.replace(tzinfo=None) - now).total_seconds() / 3600
                    else:
                        tte_hours = 0.0
    ...
    async def get_portfolio_events(self, user_id: int, days: int = 7) -> List[Dict[str, Any]]:
        """
