...
def is_whipsaw_noise(
    ...
    if not last_state or not last_state.get("last_cross_time"):
        return False
    last_dir, last_price, last_time = (
        last_state["last_cross_dir"],
        last_state["last_cross_price"],
        last_state["last_cross_time"],
    )
    now = time.time()
    ...
    if (
        last_price
        and last_price > 0
        and abs(current_price - last_price) / last_price < WHIPSAW_PRICE_BAND_PCT
    ):
...
async def validate_mtf_trend(symbol: str, trigger_sig: Dict[str, Any]) -> MTFResult:
    ...
    anchor_trend = (
        TrendState.BULLISH if price_daily > ema21_daily else TrendState.BEARISH
    )
    trigger_dir = (
        TrendState.BULLISH
        if trigger_sig.get("direction") == "BULLISH"
...
async def should_send_priority_alert(
    result: Dict[str, Any],
    ...
    for sig in ema_signals:
        if sig.get("type") == "CROSSOVER":
            if is_whipsaw_noise(
                symbol, current_price, sig.get("direction", "UNKNOWN"), last_alert_state
            ):
                continue
            mtf = await validate_mtf_trend(symbol, sig)
            if mtf.is_aligned:
                dir_cn = (
                    "多頭" if mtf.confirmed_direction == TrendState.BULLISH else "空頭"
                )
                return (
                    True,
                    f"🔥 **時框共振確認**：{symbol} 於大週期趨勢中觸發 {dir_cn} 突破",
                )
