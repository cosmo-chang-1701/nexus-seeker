...
echo "🔍 [Docker: nexus_core] 正在執行 nexus_core 容器化測試..."
cd "$ROOT_DIR/nexus_core"
...
if [ $CORE_EXIT_CODE -ne 0 ]; then
    echo ""
    echo "❌ [nexus_core] 測試失敗！"
    exit $CORE_EXIT_CODE
fi

echo ""
echo "🔍 [Docker: nexus_edge_scraper] 正在執行 nexus_edge_scraper 容器化測試..."
