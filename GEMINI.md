...
- **`services/polymarket_service.py`**: Real-time Polymarket whale monitoring service via WebSocket (`last_trade_price` channel). Features **Taker Intent Mapping** (Aggressive Long/Short, Profit Taking) and **Dual Price Calibration** (normalized Yes/No prices). Uses **Gamma API** for accurate market discovery and metadata caching. Includes robust string sanitization and connection status tracking (`/poly_status`).
