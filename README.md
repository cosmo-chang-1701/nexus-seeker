...
| 🔄 **30 分鐘動態巡邏** | 盤中掃描器以 30 分鐘心跳循環運作，僅在 NYSE 常規交易時段（09:45 ET 後）執行掃描，避開開盤洗盤期。 |
...
| **每 30 分鐘心跳** (09:45 ET – 收盤) | 盤中動態掃描 | 每 30 分鐘偵測開盤狀態，僅在常規交易時段內執行：跳過非交易日/盤前盤後/開盤洗盤期 (09:30–09:44)。對每位使用者的觀察清單執行全方位掃描；訊號推播套用 **4 小時冷卻機制**（同一使用者 × 同一標的在冷卻期內不重複推播）。 |
...
├── cogs/
│   └── trading.py           # 斜線指令、私訊分發器、動態排程任務（含冷卻機制）、Embed 格式化
├── tests/
│   ├── __init__.py
