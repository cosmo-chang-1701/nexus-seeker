...
- **`/x`**：批次量化雷達掃描。留空則展開 Unified Radar Panel (互動式控制面板)，支援多層次漏斗過濾 (如 TDP 三擊、高階磁吸過濾、暗池防護等) 及動態標籤。進階用戶亦可直接帶入 `scan_type` 參數快速 bypass 面板執行掃描。首層雷達基於快取與本地規則引擎運作，響應時間 `<100ms`。
