...
| **每 30 分鐘心跳** (10:00 ET – 收盤) | 盤中動態掃描 | 每 30 分鐘偵測開盤狀態，僅在常規交易時段內執行：跳過非交易日/盤前盤後/開盤初期造市商無報價期 (09:30–09:59)。對每位使用者的觀察清單執行全方位掃描；訊號推播套用 **4 小時冷卻機制**（同一使用者 × 同一標的在冷卻期內不重複推播）。 |
...
│   ├── test_portfolio.py    # 盤後結算引擎單元測試 (Mock 依賴)
