...
- **🩹 Max Pain 數據異常自癒與成交量降級 (Self-Healing Max Pain Fallback)**：
  - **異常監控**：當計算出的期權 Max Pain 與現貨價偏離度大於 30% 時，自動觸發警報並透過背景非同步任務重新驗證與清理快取。
  - **資料完整性退化**：自動檢測 yfinance 期權未平倉量 (OI) 資料完整性。若總合約數 > 10 但具備有效 OI 的合約數 <= 3（或佔比 < 2%），系統會判定數據源缺失，自動降級/退化為 **成交量 (Volume) 權重** 進行計算，以保證 Max Pain 結算價預估在實盤中的高可靠度與可用性。
- **🌤️ 盤前 IV 數據退化與歷史波動率代理 (Pre-market IV Fallback)**：
  - 盤前開盤前（09:30 ET 前）期權市場未開盤，無實時 IV 資料。系統會自動回退查詢 SQLite `historical_iv` 資料表之昨日收盤 IV；若無歷史快取，則自動以 30 日歷史波動率 (HV) 作為代理。
  - 若所有期權與歷史數據皆無法獲取，系統會優雅降級並於 Embed 標題自動標註 `[盤前數據未更新]`，顯示 `--%` 佔位符，防止用戶混淆。
