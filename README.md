...
- **個股深度分析面板強化 (Symbol Hub Deep-Dive Upgrades)**：`/x symbol:` 標的分析中心新增 Session VWAP、15m ATR 獨立顯示、GEX Net 曝險 Regime／個股 Gamma Flip 線／CallWall 水位與距現價空間、UOA 權利金欄位與 SWEEP/BLOCK/CROSS 啟發式三分類，並整合使用者自填的期權購買力／保證金使用率參考區塊。凡是由啟發式規則或代理指標（而非真實第一手數據）推算出的判定，一律在對應欄位旁明確標註揭露，避免與即時市場數據混淆。
