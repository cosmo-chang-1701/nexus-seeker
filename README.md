...
除了 Discord 介面，Nexus Seeker 提供獨立的 CLI 工具供開發調試與自動化排程使用。

### 核心功能
*   **即時行情查詢**：無需開啟 Discord，直接獲取 BSM 定價參考。
*   **全站量化掃描**：一鍵執行 DDP 偵測。
*   **損益報表產出**：產出美觀的終端持倉與未實現損益表格。
...
# 執行全站 DDP 掃描
python cli.py scan-ddp

# 查看特定帳戶持倉 (可切換 --db 指定資料庫)
python cli.py --user-id <UID> portfolio
```
