...
> 核心版本（nexus_core）：**v1.13.20** (請參考最新 Release)
...
- **個股深度分析面板強化 (Symbol Hub Deep-Dive Upgrades)**：`/x symbol:` 標的分析中心新增 Session VWAP、15m ATR 獨立顯示、GEX Net 曝險 Regime／個股 Gamma Flip 線／CallWall 水位與距現價空間、UOA 權利金欄位與 SWEEP/BLOCK/CROSS 啟發式三分類，並整合使用者自填的期權購買力／保證金使用率參考區塊。凡是由啟發式規則或代理指標（而非真實第一手數據）推算出的判定，一律在對應欄位旁明確標註揭露，避免與即時市場數據混淆。新增「🔐 進場鐵律檢核」頁籤，一鍵呈現「六重鐵律」的即時 Pass/Fail 判定（條件二正 Gamma 支撐牆嚴格約束於現價下方 $K < \text{Spot}$ 避免上方阻力誤判；條件一 Gamma Flip 無交叉點時根據全域 Net GEX 區分空頭阻擋或 VWAP+0.5×ATR₁₅ₘ 替代門檻），取代原本需逐欄位人工核對放量突破、正 Gamma 底牆、UOA 封頂與主力買盤確認等條件的流程。
...
- **三態交易策略與 4-Regime 決策矩陣 (Trading Strategy Modes & Regime Router)**：`/settings` 新增交易策略選單，可切換「**右側交易**」（順勢動能突破六重鐵律，維持既有預設行為）、「**左側交易**」（逆勢均值回歸／做市商 Put Wall 底牆接刀六重鐵律，含極值乖離、止跌 K 棒型態、量能窒息／恐慌吸收、主力 PUT STO 接刀認證與 Theta 磨底防禦）或「**動態調整**」。動態調整模式依 Put Wall／Call Wall／Gamma Flip／Session VWAP／ATR₁₅ₘ／RSI／量能／UOA／VIX 期限結構／大盤 Regime，將盤勢自動分類為 4 態（左側接刀態、混沌泥淖態、右側動能態、結構封頂危機態），路由至對應鐵律或強制休眠／全面鎖倉，杜絕「在追高區摸底、在接刀區追單」的邏輯衝突。職責邊界嚴格劃分：**Regime 只負責環境識別與進場權限許可**，部位的生死存亡一律回歸獨立的防洗盤風控階梯（`SL-結構失效`／`TP1-3` 等分層對所有部位一律照跑，不因部位被標記而跳過）。**狀態切換引擎**因此只保留「左側部位帶量站上 VWAP 與 Gamma Flip 時，停損上移保本並授權順勢加碼」這條真正屬於狀態轉換的路徑；上移後的保本價會持久化為停損地板交由風控階梯執行。部位歸屬透過 `/add_trade`、`/add_holding` 的選填參數手動標記，未標記部位完全不受影響。
...
- **`/settings`**：帳戶全域參數配置中心（資本、風險上限設定、虛擬交易室、Polymarket 巨鯨門檻與 AI 分析開關等），並可切換**交易策略**（動態調整／左側交易／右側交易）。
- **`/add_trade` / `/add_holding`（及對應 `/edit_*`）**：記錄實單期權與現貨持倉，可選填 `dynamic_entry_regime` 將部位標記為「動態調整」引擎託管，交由狀態切換引擎進行加碼、停損上移與防禦性平倉管理。
