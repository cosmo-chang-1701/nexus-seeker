...
### 1. 📡 Watchlist 盤中半小時心跳
於美股交易時段內，由 `SchedulerCog.dynamic_market_scanner()` 驅動，每 30 分鐘針對每位使用者訂閱的自選標的，主動推送深度戰報。其 Embed 戰報固定包含以下七大區塊：
- **📊 技術 / 期權快照**：包含價格、IV Rank / Percentile、及當日掃描之異動期權 (Unusual Option Activity, UOA)。系統會自動區分大宗交易（🔥 `SWEEP` / 📦 `BLOCK`）並計算當日 Open Interest (OI) 淨變化量。
- **📐 Skew 與市場判讀**：期權波動率偏斜與偏斜百分位數（Skew Percentile）判讀。
- **🤖 LLM Skew 解說**：串接 OpenAI 相容 API 產出精簡繁體中文分析（受系統 85% RAM 安全門檻保護，記憶體超限時自動降級）。
- **🗓️ 事件風控**：彙整即將發生的重要總經事件與財報時間。
- **💼 持倉與操作指引**：
  - **未持倉標的**：依據 RSI 與 Skew（下行恐慌貼水因子）動態算出建議買入價 (`suitable_buy_price`)，並結合使用者設定的資金與風險上限，提供建議配置股數。
...
### 3. 🔮 總經與事件日曆防護 (CME FedWatch & Calendar)
...
- **共享快取**：總經事件（以「月」快取）與個股財報（以「標的」快取）統一透過 SQLite 持久化快取，避免因重複調用外部 API 導致被封禁或增加延遲。
...
### 6. 📊 成交量分佈 (Volume Profile) 與 DDP 估值雙擊
- **V-POC 計算**：系統自動獲取歷史 K 線與成交量分佈，計算出最高密集的成交量控制點 (Volume Point of Control)。
- **DDP (Double Discount Pricing) 雙擊信號**：當標的現價同時低於 EMA 21、期權 Max Pain 以及 V-POC 三大支撐水位時，終端機會觸發「DDP 估值雙擊」專屬綠燈信號，提示絕佳的長線或價值投資買點。
...
- **`/notif_settings` 通知偏好中心**：
  - 精細控制 18 種以上通知的推送開關（預設大多開啟）。提供「⚡ 全部開啟」與「💤 全部關閉」一鍵批次設定按鈕。
...
### Discord 斜線指令表
- `/settings`：帳戶核心參數面板（資本、風險、虛擬交易室等開關與數值修改）。
- `/notif_settings`：自訂推送 preference、Polymarket 警報及一鍵開關。
- `/x`：批次量化雷達掃描（`ALL`, `HOLDINGS`, `ORDERS`, `OPTIONS`, `WATCHLIST`）。首層雷達基於快取與本地規則引擎運作，響應時間 `<100ms`。
